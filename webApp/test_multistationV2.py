#!/usr/bin/env python3
"""
test_multistation.py - VERSÃO CORRIGIDA

Teste de múltiplos sensores simultâneos
...
def sensor_worker(sensor_id: int, n_mensagens: int, resultados: dict):
    ...
    client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2)  # Fix deprecation warning
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
# TESTE 2 — Queries com múltiplos sensores
# ══════════════════════════════════════════════════════════════════════════════
...
def teste_queries_multisenso(n_sensores: int):
    ...
    with InfluxDBClient(url=INFLUX_URL, token=TOKEN, org=ORG) as client:
        ...
        tempos_individual = []
        for i in range(1, n_sensores + 1):
            # CORRIGIDO: sensor1, sensor2, etc (minúscula!)
            query = f'''
            from(bucket: "{BUCKET}")
              |> range(start: -1h)
        ...
        melhoria = (t_soma_individual - elapsed_todos) / t_soma_individual * 100 if t_soma_individual > 0 else 0
...
def teste_perda_dados(n_sensores: int, n_mensagens: int):
    ...
    # IMPORTANTE: Guardar timestamp de quando COMEÇOU o teste
    # para contar só os pontos DESTE teste, não dados antigos
    tempo_inicio_teste = time.time() - 30  # 30s atrás (para cobrir o tempo de publicação)

    with InfluxDBClient(url=INFLUX_URL, token=TOKEN, org=ORG) as client:
        for i in range(1, n_sensores + 1):
            # CORRIGIDO: sensor1, sensor2, etc (minúscula!)
            # E usar timestamp do início do teste para não contar dados antigos
            query = f'''
            from(bucket: "{BUCKET}")
              |> range(start: {int(tempo_inicio_teste)})
              |> filter(fn: (r) => r["_measurement"] == "sensor{i}")
              |> filter(fn: (r) => r["_field"] == "LAEA")
              |> count()
            '''
            ...
            # Status baseado em 90% (não 95% porque pode haver pequenas perdas normais)
            status = "✅" if count >= n_mensagens * 0.90 else ("⚠️" if count >= n_mensagens * 0.70 else "🔴")
            percentagem = (count/n_mensagens*100) if n_mensagens > 0 else 0
    ...
    if taxa >= 95:
        print(f"  🟢 Excelente! Sem perda de dados")
    elif taxa >= 85:
        print(f"  🟡 Boa taxa de entrega (perda <15%)")
    elif taxa >= 70:
        print(f"  ⚠️  Alguma perda de dados — investigar pipeline")
    else:
        print(f"  🔴 Perda significativa — problema no sistema")

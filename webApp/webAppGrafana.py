...
# ============================================================================
# ========== Funções Auxiliares para escolher que bucket utilizar   ==========
# ============================================================================
...
def pode_usar_bucket_agregado(duration_days, field=None):
    """
    Verifica:
    1. Duração > 7 dias?
    2. Campo está na lista de agregados?
    
    Só retorna True se AMBOS forem verdade
    """
    if duration_days < 7:
        return False
    if field is None:
...
@app.route('/acesso-tecnico', methods=['GET', 'POST'])
def login():
...
def obter_dados_brutos():
    ...
    # Escolha de Bucket:
    # EventDetect e EventType1-10 nao estão no bucket agregado, só no raw, porque são eventos discretos, não faz sentido agregar
    bucket_name = bucket 
    measurament = sensor

    duration = end_dt - start_dt
    duration_days = duration.total_seconds() / (3600 * 24)
    print(f"[api/eventos] Query {duration_days:.2f} dias -> bucket raw")
    # Campos a procurar:
...
def get_raw_data():
    ...
    """ "laea": f'''
        ...
        ''' """
    queries = {}
    # LAEA - pode usar bucket agregado
    if pode_usar_bucket_agregado(duration_days, "LAEA"):
        bucket_name = "SoundDashHosp_hourly"
        measurement_name = f"{measurement}_hourly"
        print(f"[/api/stats] LAEA: {duration_days:.1f} dias → bucket agregado")
    else:
        bucket_name = bucket
        measurement_name = measurement
        print(f"[/api/stats] LAEA: {duration_days:.1f} dias → bucket raw")
    ...
    # LCpeak - NÃO está agregado, sempre bucket raw
    bucket_name_lcpeak = bucket
    measurement_name_lcpeak = measurement
    print(f"[/api/stats] LCpeak: {duration_days:.1f} dias → bucket raw ")
    
    queries["lcpeak"] = f'''
        from(bucket: "{bucket_name_lcpeak}")
          |> range(start: time(v: "{start}"), stop: time(v: "{end}"))
          |> filter(fn: (r) => r["_measurement"] == "{measurement_name_lcpeak}")
          |> filter(fn: (r) => r["_field"] == "LCpeak")
          |> aggregateWindow(every: {window}, fn: max, createEmpty: false)
          |> limit(n: {limit})
    '''
    
    # LAFmax - NÃO está agregado, sempre bucket raw
    print(f"[/api/stats] LAFmax: {duration_days:.1f} dias → bucket raw")
    queries["lafmax"] = f'''
        from(bucket: "{bucket_name_lcpeak}")
          |> range(start: time(v: "{start}"), stop: time(v: "{end}"))
          |> filter(fn: (r) => r["_measurement"] == "{measurement_name_lcpeak}")
    ...
    # LAFmin - NÃO está agregado, sempre bucket raw
    print(f"[/api/stats] LAFmin: {duration_days:.1f} dias → bucket raw")
    queries["lafmin"] = f'''
        from(bucket: "{bucket_name_lcpeak}")
          |> range(start: time(v: "{start}"), stop: time(v: "{end}"))
          |> filter(fn: (r) => r["_measurement"] == "{measurement_name_lcpeak}")
...
def download_csv():
    ...
    # ===== ESCOLHA DE BUCKET =====
    # Download CSV precisa de TODOS os campos (agregados + não agregados)
    # Sempre usar bucket raw para ter todos os campos disponíveis
    bucket_name = bucket  # SoundDashHosp
    measurement_name = measurement
    
    duration = end_dt - start_dt
    duration_days = duration.total_seconds() / (24 * 3600)
    print(f"[/api/download] Query {duration_days:.1f} dias → bucket raw (precisa todos campos)")
...
def grafana_proxy(path):
    ...
    # Passa os query parameters também
...
#Rotas para obter e atualizar parâmetros de sensores (via MQTT)
...
def update_parametros():
    ...
    print("[Flask] ========= POST /api/parametros =========")
    print(f"[Flask] Request method: {request.method}")
    print(f"[Flask] Request content type: {request.content_type}")
    print(f"[Flask] Request data: {request.data}")
    print(f"[Flask] Request JSON: {request.get_json()}")
    print(f"[Flask] ====================================")
...
def check_auth():
    ...
    Retorna 200 se user autenticado, 401 caso contrário
    """
...
def get_data():
    ...
    try:
        ...
        # Escolher bucket e measurement baseado no intervalo E no campo
        if pode_usar_bucket_agregado(duration_days, field):
            ...
            print(f"[/api/data] {field}: {duration_days:.1f} dias → bucket agregado")
        else:
            bucket_name = "SoundDashHosp"
            measurement = f"sensor{sensor_id}"
            if duration_days > 7:
                strategy = f"raw data (field '{field}' not aggregated)"
                print(f"[/api/data] {field}: {duration_days:.1f} dias → bucket raw (campo não agregado)")
            else:
                strategy = "raw data (1s resolution)"
                print(f"[/api/data] {field}: {duration_days:.1f} dias → bucket raw")

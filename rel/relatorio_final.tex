...
\subsection{Gestão do Reinício Após Atualização de Config}

Quando a estação recebe e aplica uma atualização de configuração, agenda um \texttt{sudo reboot} após 5~segundos, garantindo que o ACK de confirmação é enviado à aplicação antes do sistema reiniciar. O reboot completo é necessário para garantir que todos os parâmetros (incluindo broker MQTT, taxa de amostragem e caminhos de modelo) são relidos com o novo estado do sistema operativo.

Do lado da aplicação web, após o envio de uma configuração com sucesso, é apresentado um temporizador de contagem decrescente de 2~minutos que desativa o botão de verificação de configuração (``↺ Atualizar''). Este mecanismo evita que o utilizador tente ler a configuração da estação durante o arranque, quando o Tailscale ainda pode não estar completamente ligado e a estação não responde ao canal MQTT. Após os 2~minutos, o botão é reativado automaticamente e o utilizador pode verificar que a nova configuração foi aplicada com sucesso.
...
Countdown pós-config        & Temporizador 2 min na app após envio      & Completo \\
